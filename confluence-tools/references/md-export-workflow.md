...
- code 宏 → 语言围栏；原生 `toc` 和 Easy Heading 目录宏直接省略；
  note/info/warning → 引用块。
- 未知宏保留可读正文，并附带 HTML 转义的原始 Confluence XHTML 注释；
  注释中的 `--` 转为 `&#45;&#45;`。
- `ri:url`、外链图片和未知宏正文中的独立 URL 保留。
- 代码和公式使用占位符穿过正文空行规范化；代码恢复后禁止再次全局整理空行。
- 单列且每格只有代码块的表格按行拆成独立围栏代码块；多列或代码与说明混合的表格按
  “第 N 行 + 列标签 + 内容”纵向展开，避免 Typora 把 HTML 标签和 Shell 变量渲染为正文或公式。
- 多列纯文本表格继续输出 GFM 管道表格；不规则含代码表格按实际单元格顺序展开，立即提示并在结束时汇总。
...
- 页面目录默认只使用页面标题。
- 同一导出层级存在同名标题，或标题清理后发生目录名冲突时，冲突组内全部页面改用
  `<页面ID>_<页面标题>`；脚本立即提示冲突页面 ID，并在运行结束时汇总，禁止静默覆盖。
...
- 无参数运行时从 `export_config.default_page` 或 `export_config.space` 读取默认范围；
  两者不能同时设置。
- `--page-id`：导出单页；`--recursive` 时递归导出后代并嵌套目录。
- `--space`：空间索引已经包含全部页面，按平铺方式每页导出一次；不得对每页再次递归。
- 显式 `--page-id` 或 `--space` 覆盖对应的默认范围；同一次执行不允许页面和空间范围同时生效。
