...
1. 使用用户指定的 Markdown 文件或目录，并直接读取 `import_config`；不重复询问配置已有值。
   人工运行可显式传 `--manual`，按 `manual_run_config.md_import_mode` 选择 `file`（默认，
   读取 `md_import_file`）或 `tree`（读取 `md_import_dir`）。该开关与位置参数、`--dir`、
   `--resume` 互斥；文件必须是已有 `.md`，目录必须存在且开启 `import_config.tree_import`，
   并在远端操作前显示完整路径。树模式复用 `--dir` 流程，支持 `--plan-only`、`--yes`
   和 `--fix-hierarchy`；例如 `md_import.py --manual --plan-only` 先预览目录树计划。
...
3. 先计算最终 `preflight_review`。开启时，按 `SKILL.md` 的直接路由，在任何远端查询或写入前
   同时完整读取 `md-import-preflight-rules.md`，只读预审全部待写文件。助手同时按入口直接路由的
   `md-import-splitting-rules.md` 全量评估容量；格式通过且无需拆分时才直接上传原件。需要修复
   或拆分时在同级完整 `__修复` 副本中处理，并执行“全部受影响文件通过 → 最终目录全量通过”
   的两级复审。关闭格式预审不等于关闭容量评估；关闭时明确提示风险。
...
- 需要修复或拆分时先完整复制用户指定的根文件夹；初始名称、附件和层级与原目录一致。
  拆分后按最终文件生成页面树，原逻辑根标题不带 `__修复`。已拆分整章备份放在上传范围外。
- 修复副本重名时必须询问覆盖、另起同级名称或取消；不得静默覆盖。状态报告放在副本旁边，
  不放入待上传目录。
- 助手流程：全目录格式与容量评估 → 必要时副本修复／均衡拆分 → 两级复审 →
  按最终文件扫描建树 → 一次拉取空间索引 →
...
- `fix_hierarchy=confirm`：存在移动时每次展示计划并等待确认；`auto`：直接移动；
  `off`：更新正文但不移动。
- `--plan-only` 只审核并生成计划，不创建副本、不拆分、不执行远端写入；发现待修复问题时停止。
  助手先列出全目录容量与拟拆分清单；该命令本身不提供自动拆分功能。
...
- 报告页面 ID、版本以及成功/失败汇总。
- 树导入核对页面数量和父子关系；附件失败不得隐藏。拆分导入还要核对正文顺序、公式数量、
  图片引用、目录和实际渲染锚点；保存源章节／分段文件／页面 ID 对应表。
- 内容核验和加载性能分开报告；不得用 API 成功或容量下降代替浏览器性能验证。
