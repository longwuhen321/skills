...
# ==================== md_import 配置 ====================
import_config = {
    # 代码块折叠阈值：非负整数，实际行数大于此值时折叠，否则展开。
    # 忽略首尾空白行；内部空行和注释计入。0 表示折叠所有非空代码块。
    "code_collapse_threshold": 10,

    # 本地图片附件的默认显示宽度（px）：非负整数，0 表示不统一设置。
    # 源尺寸优先（路径尺寸参数、HTML width/height 属性或像素样式）；
    # 无源尺寸时只补宽度，高度按比例缩放，不修改原图。单页和树导入共用。
    "image_width": 750,
