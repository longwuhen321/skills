...
- **禁止使用 Codex 的选项式提问工具**（如 `request_user_input`）：其选项形式适合选择，不适合 URL/Token/路径等开放输入，且会诱导把扫描结果硬塞进选项（违反规则 2 边界）
...
配置确认后，如执行该 Python 路径需要超出当前沙箱权限，必须先通过 Codex 的权限审批；如需免除后续重复确认，通过 Codex 的审批界面设置对应规则（避免后续执行每次确认）。
