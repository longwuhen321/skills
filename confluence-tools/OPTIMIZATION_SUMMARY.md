...
### 2026-09-18：导入图片默认显示宽度配置

| 类别 | 内容 |
|------|------|
| 配置 | 新增 `import_config.image_width`，模板与实际配置均为 750 px；0 关闭默认宽度，非负整数以外的值在网络会话和日志初始化前拒绝 |
| 脚本 | 单页与树导入共用现有图片转换逻辑；本地图片无源尺寸时补 `ac:width`，路径尺寸参数、HTML 宽高属性和像素样式优先；已有高度时不补默认宽度 |
| 边界 | 只控制显示尺寸，不重采样原图；外链、data URI、代码和宏保持原有处理方式；不自动回溯已有页面，不改变命令参数、页面树或公式策略 |
| 文档 | 同步入口、配置指南、脚本开发规则、配置模板和实际配置 |
| 验证 | 基线 232 项自测通过，增加 5 项测试后 237 项通过；配置同步、依赖、四入口 help、UTF-8/LF、Python 语法与隔离发布检查通过 |

**过程要点**：
- 默认宽度只在源图没有任何明确宽高时生效，避免给已有高度的图片补宽度后造成变形。
- 新配置的导入行为通过离线转换与 mock 验证，未为此新建远端测试页；已上传页面的尺寸调整单独按指定页面 ID 执行并回读核对。

**遗留事项更新**：
- 配置调整不自动修改既有页面，已有页面需另行指定处理范围；浏览器实际阅读效果由用户验收。

### 2026-09-17：助手导入前全目录容量评估与均衡拆分

| 类别 | 内容 |
|------|------|
| 流程 | 全目录容量评估、内容边界优先、均衡拆分、完整副本及最终文件建树；格式三态不变 |
| 文档 | 同步入口、导入流程和预审规则，新增直接路由的 md-import-splitting-rules.md |
| 边界 | 仅助手工作流；导入脚本、配置、CLI 和触发契约不变；不增加直接覆盖已有副本的通用许可 |
| 验证 | 232 项 selftest、七组配置同步、依赖、四入口 help、隔离发布检查、文档路由及 UTF-8/LF 检查通过；真实书籍 9 份全量评估、拆分后 17 份全部 CLEAN，逐段还原与原文一致；17 页回读、6937 个公式宏、136 个图片引用和 68 个目录链接通过；加载和渲染效果由用户验收 |

**过程要点**：
- 格式 CLEAN 与页面容量分别判断，避免内容正确但过长的页面直接上传。
- 计数单位明确标注，原始正文、公式和附件引用守恒，页面计划绑定最终文件和实际 ID。

**遗留事项更新**：
- 独立运行导入脚本仍不自动拆分；不把文档流程扩展描述为脚本功能。

### 2026-08-19：人工单文件默认配置与 `--manual` 安全入口

| 类别 | 内容 |
|------|------|
| 新功能 | 新增独立 `manual_run_config.md_import_file`，供人工执行 `md_import.py --manual` 时选择默认 Markdown 文件；AI 助手仍显式传入文件或目录范围，不读取人工默认值 |
| CLI 与优先级 | `--manual` 必须显式启用，并与 Markdown 位置参数、`--dir`、`--resume` 互斥；显式文件路径保持原有行为且不加载人工默认值 |
| 写入安全 | 人工配置为空、无法解析、文件不存在或扩展名不是 `.md` 时，在创建导入器和任何 Confluence 操作前失败；使用配置时显示解析后的完整路径 |
| 配置与文档 | 配置同步门禁由六组扩展为七组；同步 `config.example.py`、真实 `scripts/config.py`、`SKILL.md`、配置向导、导入流程、维护规则和脚本开发规则 |
| 测试与验证 | 新增显式文件忽略人工配置、有效人工文件、来源互斥及非法路径用例；完整 selftest 由 223 增至 227 项并全部通过，七组配置同步、四入口 help、依赖、AST 语法、LF、`git diff --check` 和隔离发布检查均通过 |

**过程要点**：
- 导入属于远端写操作，脚本无法可靠识别调用者是人工还是 AI，因此没有让无参数运行自动读取默认文件，而是使用显式 `--manual` 门禁。
- `manual_run_config` 只承载人工入口文件路径；空间、父页面、页面标题和转换策略仍由 `import_config` 管理，避免重复配置。
- 真实配置中的 `md_import_file` 初始留空，由用户按需填写绝对路径；本次未连接或修改 Confluence。

**遗留事项更新**：
- （新增）无。

### 2026-08-13：Markdown 导出目录按需使用 Page ID

| 类别 | 内容 |
|------|------|
| 导出行为 | 无同层重名时页面目录由 `<page ID>_<标题>` 简化为 `<标题>`；同层标题相同或文件名清理后冲突时，冲突组全部使用 `<page ID>_<标题>` |
| 冲突安全 | 在同层页面写入前统一规划目录名，不依赖遍历顺序；即时打印冲突标题和全部 Page ID，结束时汇总冲突组、父目录与页面 ID，避免实际覆盖 |
| 范围 | 单页、递归页面树和空间平铺导出共用规则；已有导出目录不回溯重命名，storage 调试快照仍保留 Page ID |
| 文档 | 同步 `SKILL.md`、`references/md-export-workflow.md` 和 `references/script-development-rules.md` |
| 测试 | 修改前 217 项通过；更新普通页面和递归目录断言，新增递归同名、空间同名、清理后重名及冲突汇总用例，修改后 221 项通过 |

**过程要点**：
- 同名组中的所有页面统一带 Page ID，避免“第一个页面无 ID、后续页面带 ID”造成结果依赖 API 返回顺序。
- 目录冲突按文件名清理后的结果且忽略大小写判断，覆盖 Windows 文件系统的实际碰撞语义。

**遗留事项更新**：
- 已经导出的目录按用户要求不回溯修改；重新导出后使用新命名规则。

### 2026-08-13：Markdown 导出省略 Confluence 目录宏

| 类别 | 内容 |
|------|------|
| 导出行为 | `md_export.py` 不再把原生 `toc` 转成 `[toc]`，也不再把 `easy-heading-free` 当作未知宏输出 XHTML 注释；两类目录宏在 Markdown 导出中统一省略 |
| 内容安全 | 只删除目录宏节点；宏嵌在 H1～H6 内时保留父标题和后续正文 |
| 日志核对 | 最新 `logs/export/20260813_143325` 含 2 个 storage 快照；其中页面 `12058630` 含 1 个无正文 body 的 `easy-heading-free` 宏，可安全省略 |
| 文档 | 同步 `SKILL.md`、`references/md-export-workflow.md` 和 `references/script-development-rules.md`，明确导出不生成 Markdown 目录标记或目录宏注释 |
| 测试 | 新增 Easy Heading 省略用例，并把原生 toc 根级/标题内用例改为省略语义；修改前 216 项通过，修改后 217 项通过 |

**过程要点**：
- 最新页面实际使用 Easy Heading，而旧导出器只识别原生 `toc`；因此旧输出既可能出现 `[toc]`，也可能出现“未处理的宏”注释，需要同时处理两种宏名。
- 不增加配置开关，直接将“目录属于 Confluence 展示结构”固化为导出语义，避免 Markdown 阅读器兼容性差异。

**遗留事项更新**：
- 已经导出的 Markdown 按用户要求不回溯修改；后续重新导出时自动应用新行为。

### 2026-08-13：通用目录宏与可配置树根页面

| 类别 | 内容 |
|------|------|
| 通用 TOC 配置 | 新增 `common_config.toc_target_macro`，`md_import.py` 的 `[toc]` 与自动目录、`toc_upgrade.py` 的默认转换目标共用 `easy_heading` / `toc`；CLI `--target` 仍优先，旧 `toc_upgrade_config.target_macro` 保留兼容读取 |
| 共享宏逻辑 | Easy Heading 参数校验、原生 TOC / Easy Heading 构造和已有目录检测收敛到 `common.py`；显式标记和自动插入不再硬编码原生 `toc`，同时识别两类目录以防重复 |
| 树根页面 | 新增 `import_config.materialize_root_page`。开启时根文件夹始终成为页面：同名 `.md` 作正文，缺失时用合法最小空正文创建 Confluence 页面且不修改本地目录；根级其他 `.md` 各自作为直接子页面 |
| 已有根页安全 | 合成根页面已存在且无需移动时直接复用，不用空正文覆盖已有内容；需要移动时先读取并保留当前 storage，版本变化则停止并要求重新生成计划 |
| 配置与文档 | `config.example.py` 默认关闭根页以兼容旧行为；当前 `.codex` 实际配置启用根页并以 Easy Heading 为通用目标。同步 `SKILL.md`、配置指南、导入与目录升级工作流 |
| 测试与验证 | 修改前 selftest 204 项通过；新增通用目标、重复目录、合成根页、根级 Markdown、空正文创建、已有根页复用和保留正文移动用例，修改后 selftest 212 项通过。未执行真实 Confluence 写入 |

**过程要点**：
- 根页开关不能只创建空父页；否则 `ROS2/课程简介.md` 会被吞作根页正文或被忽略，因此开启后将非同名根级 Markdown 显式建成子节点。
- 合成根页只存在于导入计划和 Confluence，不生成 `<根目录名>.md`，继续遵守原件只读与修复副本规则；`synthetic` 标记随 `tree_plan.json` 保存，可断点续传。
- `.codex` 与 `.agents` 是独立副本，本次只修改 `C:\Users\MSI\.codex\skills\confluence-tools`。

**遗留事项更新**：
- 真实 Confluence 页面创建、已有合成根页移动与 Easy Heading 渲染仍需在明确授权的受控范围验证；本次只做离线 mock 与静态门禁。

### 2026-08-13：Markdown 原件直传与同级完整修复副本

| 类别 | 内容 |
|------|------|
| 预审状态 | `md_preflight.py` 明确输出 `CLEAN` / `FIXABLE` / `BLOCKED`；审核候选只保留在内存，`logs/` 仅写 `review.json` 与 `manifest.json`，不再生成供修改或上传的 `candidate.md` |
| 原件与副本 | 单文件或目录先只读全量审核；全部干净时直接上传原件。发现任一问题时在同级创建 `<原名>__修复` 完整副本，目录内部文件、附件、名称和层级初始保持一致；只修改副本，原件字节不变 |
| 冲突与标题 | 修复副本已存在时交互选择覆盖、另起同级名称或取消，禁止静默覆盖；副本旁的 `.confluence-review.json` 保存原逻辑名称和待复审文件，`__修复` 不进入 Confluence 页面标题 |
| 两级复审 | 先复审全部问题文件，全部通过后再全量复审整个副本；全量审核发现新问题时返回问题文件阶段。最终审核对象在远端写入前按完整文件集合复核 SHA256，变化即停止 |
| 复盘规则 | 通用问题按维护流程固化到审核/转换脚本并补回归测试；仅当前内容成立的个例先询问用户是否写入 `KNOWN_ISSUES.md` |
| 测试与验证 | 新增原件直传、单文件修复、完整目录复制、问题文件优先、页面标题、冲突改名、路径越界和审核后变更用例；完整 selftest 204 项通过，六组配置同步、四依赖、编译、四入口 help、LF、差异及隔离发布检查通过 |

**过程要点**：
- 修复状态文件放在副本旁边而不是副本内部，既能跨次运行恢复问题文件清单，又不污染待上传目录结构。
- `--plan-only` 保持只读：只审核原件并生成计划；发现问题时停止，不创建修复副本。
- 本次仅做离线脚本、临时目录和文档门禁验证，没有连接或修改 Confluence，也没有读取真实 `logs/`。

**遗留事项更新**：
- （原）审核 Markdown 放在 `logs/` 中导致 AI 修改成本高 → **已解决**，日志不再承载可编辑候选，实际修复只发生在原件同级副本。
- （新增）本次未发现只针对某一上传内容的个例错误，因此没有新增 `KNOWN_ISSUES.md` 条目。

### 2026-08-12：SKILL.md 渐进式披露与强制路由

| 类别 | 内容 |
|------|------|
| 入口瘦身 | 将 `SKILL.md` 从 466 行压缩为 204 行；入口只保留触发说明、任务路由、公开读取回执、执行门禁、命令速查和全局安全红线，详细配置与工作流移入直接路由的参考文档 |
| 文档拆分 | 新增 `configuration-guide.md`、`md-import-workflow.md`、`math-upgrade-workflow.md`、`toc-upgrade-workflow.md`、`md-export-workflow.md` 和 `maintenance-rules.md`；为原有 `md-import-preflight-rules.md`、`script-development-rules.md` 补齐统一的“强制读取条件”与用户可见回执 |
| 防略读机制 | `SKILL.md` 使用单层直接路由：要求在调用页面脚本、修改文件或连接服务器前完整读取全部命中文档；缺失、读取不完整或规则冲突时停止，并按固定格式向用户公开任务类型、已读文件、范围和执行边界 |
| 安全保留 | 页面范围、写入前验证、批量失败、并发版本、凭据、XHTML、真实测试清理、权限、临时产物和 HTTP 重试仍保留在入口；参数全集、流程细节、脚本恢复和发布门禁移至对应权威参考文件 |
| 测试与验证 | 新增 `TestSkillDocumentationRoutes` 4 项回归，机械检查入口为 180～240 行、8 个直接引用存在且含强制完整读取措辞、核心红线未丢失及上位维护标准可达；完整 selftest 196 项通过，六组配置同步与依赖检查通过 |

**过程要点**：
- 采用“入口直接路由 + 参考文档强制读取条件 + 用户可见回执 + 回归测试”四层约束，降低长入口被略读和多级引用漏读的风险。
- 条件性预审规则也由入口直接列出，避免依赖 workflow 再转述下一层文档；核心安全规则允许在入口和工作流有意重复。
- 本次只调整 skill 文档结构及文档契约测试，没有修改运行期脚本、配置值或连接 Confluence 服务器。

**遗留事项更新**：
- （原）`toc_upgrade` 微秒时间戳日志尚未被清理器识别 → **保持未处理**，仍按独立 bug 流程修复，不混入本次文档结构优化。
...
| 类别 | 内容 |
|------|------|
| 配置与流程 | `import_config.preflight_review` 默认开启；单页和树导入在任何远端写入前审核全部 Markdown，源文件字节不变，只把 `candidate.md` 交给正文转换，附件仍从源目录解析 |
| 审核产物 | 新增 `scripts/md_preflight.py` 和按需规则 `references/md-import-preflight-rules.md`；产物写入 `logs/intermediate/<时间>/<任务>/`，成功归档到 `logs/_archive/`，失败保留 `candidate.md`、`review.json`、`manifest.json` |
| 公式安全 | 支持对称空格 `$ x $`；仅对含 LaTeX 命令、上下标或明确算术运算的单边空格公式做副本级修复；模糊 `$ x$` / `$x $`、shell 变量和金额继续阻断。审核花括号、环境、分隔符、附件、XHTML、宏数量、残留占位符和数学宏内 emphasis |
| 验证 | 16 个 Kalman Filter Markdown 全部离线预审和 storage 验证通过，3 处高置信度单边空格只在副本修复；仅更新 `22708986` 下映射的 16 页，远端逐页对账 16/16 通过 |

**过程要点**：
- 树导入先全量预审再写第一页，避免中途发现坏源文件后留下不完整树。
- 块公式宏先用内部占位符保护，防止 `\colorbox{...}{$...$}` 内部美元符被二次转换成行内宏。
- `--resume` 初始化时禁用日志清理，避免待恢复 checkpoint 在读取前被保留策略删除。
- Confluence storage 要求 `<br>` / `<hr>` 自闭合；转换器统一规范化，预审器增加专用门禁。

**遗留事项更新**：
- 预审默认开启并已完成受控子树实机验证；其他页面未修改。

### 2026-08-12：标题公式存储模式配置化

| 类别 | 内容 |
|------|------|
| 配置与脚本 | `common_config.heading_math_mode` 新增 `literal` / `mathinline` 两值枚举，默认 `literal`；`md_import.py` 与 `math_upgrade.py` 通过 `common.py` 的统一校验和标题归一化逻辑读取同一值，非法值立即失败 |
| 模式语义 | `literal` 保持 Confluence 9.2.1 已验证的标题 `$...$` / 正文宏行为；`mathinline` 将标题字面公式和旧行内 mathjax 宏统一为原生行内宏，且不套用正文复杂公式转块宏的启发式规则 |
| 验证 | 新增导入与升级双模式、旧宏、非法值、宏数量校验和二次运行幂等测试；完整 selftest 增至 159 项并通过（最终门禁见本次交付报告） |

**过程要点**：
- 配置放在 `common_config`，避免导入与升级使用不同策略后互相回退。
- 配置名使用枚举而非布尔值，使 storage 目标格式自描述；`$$...$$` 不作为标题模式，因为它是块公式定界符。

**遗留事项更新**：
- （原）目录标题公式固定为字面 `$...$` → **已配置化**，默认行为不变。
- （新增）`mathinline` 模式用于其他 Confluence 版本或插件环境；在 9.2.1 的 toc 宏中仍可能复现已知错位，因此不作为默认值。
