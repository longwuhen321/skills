...
- **根因**：`md_import.py` 对全文统一执行 `$...$ → mathinline`，未区分 H1~H6；`math_upgrade.py` 只保护标签本身而非完整标题内容，其零残留校验也把标题中的有意 `$...$` 视为失败。导入、升级之间缺少统一的标题公式语义。
- **修复**：`scripts/common.py` 新增公共标题行内公式归一化，仅在 H1~H6 中把 `mathinline` / 旧行内 mathjax 宏还原为字面 `$...$`；`md_import.py` 在最终 storage 清理后调用；`math_upgrade.py` 在通用转换前修复旧标题宏、完整保护标题、调整宏数量校验与变更统计，并在残留检查中只豁免标题区域。正文公式宏逻辑保持不变。
- **排查方法**：目录公式异常时先查看 storage 的 H1~H6 内是否含 `ac:name="mathinline"`；运行 `scripts/test/selftest.py`，重点检查标题导入+toc、旧标题宏修复、零残留豁免、二次升级幂等及导出往返用例。
