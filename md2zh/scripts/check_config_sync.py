...
def check_config_sync(example_text, config_text, group=DEFAULT_GROUP):
    ...
    """
    example_types = parse_config_text(example_text, group)
    config_types = parse_config_text(config_text, group)
...
def main(argv=None):
    ...
    try:
        ...
        example_types = parse_config_text(example_text, args.group)
    ...
    try:
        ...
        config_types = parse_config_text(config_text, args.group)
    except ConfigUnverifiable as e:
        print(f"❌ 真实配置无法解析: {e}")
        return 2

    problems, config_types = check_config_sync(example_text, config_text, args.group)
