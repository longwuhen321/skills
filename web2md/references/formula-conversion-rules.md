...
阶段 C 通读命中 `SKILL.md` 分类索引中的 A–I、S 或 F2 类候选时，
**必须读取对应章节**；不要求完整读取未命中的符号表。仅需流程纪律而没有
LaTeX 转换候选时不读取本文件。

本文件只承载转换规则表；“判断由 AI 助手完成、脚本只做精确替换”等流程纪律
保留在 `SKILL.md`。

## 目录

- A–H：§1.1–§1.8 斜体、粗体、函数与混合表达式
- I：§2.1–§2.9 Unicode 数域、单位、运算符、关系、集合、箭头与边界
- S：§3 Sphinx 遗留公式模式
- F2：§4 碎片化行内公式序列
- 辅助扫描：§5
