...
### 2026-08-19：SKILL.md 渐进披露与直接路由重构

| 类别 | 内容 |
|------|------|
| 入口精简 | `SKILL.md` 从 410 行压缩为 191 行，只保留显式触发、基础抓取、公式/结构审核、核心红线和直接路由；正常单页流程不再加载配置向导、导航降级和维护历史细节 |
| 一级参考 | 新增 `configuration-guide.md`、`children-collection-workflow.md`、`maintenance-rules.md`；现有自定义站点、公式转换和脚本开发参考补充“强制读取条件”或“按索引读取条件”，超过 100 行的工作流参考增加目录 |
| 路由约束 | 借鉴 `confluence-tools` 的入口设计，把首次配置、子页面、非平台公式、分类公式、skill 维护、脚本修改、bug 与优化八类情形全部从 `SKILL.md` 直接路由；不依赖参考文件之间的二级发现 |
| 历史校正 | `KNOWN_ISSUES.md` 新增覆盖说明：现行 `fix_escapes.py` 只修改已配对公式内的 `\_` / `\*`，散文与代码不动；保留 2026-08-01 旧条目作为历史证据，不再把它当现行规范 |
| 测试与验证 | `test_remaining_optimizations.py` 新增 5 项文档路由回归，固定 240 行硬上限、直接引用、读取条件、长参考目录、核心红线与显式调用开关；完整 selftest 108/108、配置同步 7/7、skill 快速校验、`git diff --check`、10 个变更文件 LF 检查和 28 文件隔离发布检查均通过 |

**过程要点**：
- 低频执行细节只移动到 `references/`，运行脚本、真实配置和默认行为均未改动；入口仍保留“配置失败即停止”“无法核实不等于没有子页面”“公式/代码载荷不可改写”“AI 全量审核”“决定不改也记录”和 final_verify 零 FAIL/REVIEW 等硬门禁。
- 公式规则保留按 A–I、S、F2 分类索引读取，避免为了一个符号候选加载完整 200 余行规则；其他工作流参考要求命中后完整读取，二者在入口中明确区分。
- Windows 下 skill 快速校验器默认用系统编码读取文件会触发 GBK 解码错误；改用同一解释器的 `-X utf8` 启动模式后通过，不修改 UTF-8 文档去迎合平台默认编码。
- 文档架构被纳入现有离线 selftest，后续若入口重新膨胀、直接路由丢失、读取条件弱化或隐式调用被打开，会立即回归失败。

**遗留事项更新**：
- （原）`SKILL.md` 运行细节、配置、导航与维护规则重复堆叠 → **已完成一级参考拆分**，入口稳定在 191 行，并由 240 行硬上限约束。
- （原）`fix_escapes` 历史说明与现行公式限定实现冲突 → **已通过不可变历史上的覆盖条目澄清**。
- （新增）不为行数目标继续拆分基础公式审核；只有出现新的独立低频工作流时才新增一级参考，避免渐进披露反向演变成引用碎片化。
