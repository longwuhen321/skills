...
3. **按「实现前检查点」输出方案，让用户确认后再动手**：
   ...
   - **工具链密度原则（skill 自我进化专属）** — 修改 skill 自身的机制（非用户一次性需求）时，按 2026-08-09 codex 合并沉淀的标准：为真实失败场景设计完整方案（防御边界、冲突默认拒绝、身份先落、级联失效、三态代替布尔），而不止于清单的最小项；测试用能力矩阵 + 真实端到端（本地服务器/完整 pipeline），不只 selftest。判别标准：该机制是否服务真实失败场景，不为做而做（参考三个 skill 的 2026-08-09 OPTIMIZATION_SUMMARY 条目）
